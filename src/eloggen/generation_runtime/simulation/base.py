...
from eloggen.generation_runtime.interface_names import (
    ...
    install_dataset_interface_metadata_reader,
)


# Keep inherited dataset metadata reads on the same canonical EG_* naming path
# as runtime interface lookup. This also covers direct generation-runtime entry
# points because this module is imported before interface metadata is consumed.
install_dataset_interface_metadata_reader()
...
def make_interface(name, interface_type, *args, **kwargs):
    ...
    """
    name = canonicalize_env_interface_name(name)
...
def register_env_interface(cls):
    ...
    """
    ignore_classes = ["EG_EnvInterface"]
...
class EG_EnvInterfaceMeta(type):
    """
    This metaclass adds env interface classes into the global registry.
    """
    def __new__(meta, name, bases, class_dict):
        cls = super(EG_EnvInterfaceMeta, meta).__new__(meta, name, bases, class_dict)
        register_env_interface(cls)
        return cls


class EG_EnvInterface(metaclass=EG_EnvInterfaceMeta):
    """
    Environment interface API that generation environment interfaces should conform to.
    """
    ...
    def target_pose_to_action(self, target_pose, relative=True):
        ...
        Args:
            target_pose (np.array): 4x4 target eef pose matrix
    ...
    def action_to_target_pose(self, action, relative=True):
        ...
        Returns:
            target_pose (np.array): 4x4 eef pose matrix
        """
        raise NotImplementedError
    ...
    def get_object_poses(self):
        """
        Gets the pose of each object relevant to trajectory generation in the current scene.
