...
def write_episode_metadata_json(ep_data_grp, episode_metadata):
    if episode_metadata is None:
        return

    metadata_json = json.dumps(episode_metadata, ensure_ascii=False, sort_keys=True)
    ep_data_grp.attrs["metadata_json"] = metadata_json
    if "grasp_order" in episode_metadata:
        ep_data_grp.attrs["grasp_order"] = json.dumps(
            episode_metadata["grasp_order"],
            ensure_ascii=False,
        )
    if "phase_execution_order" in episode_metadata:
        ep_data_grp.attrs["phase_execution_order"] = json.dumps(
            episode_metadata["phase_execution_order"],
            ensure_ascii=False,
        )

    dt = h5py.string_dtype(encoding="utf-8")
    metadata_grp = ep_data_grp.require_group("metadata")
    metadata_grp.attrs["json"] = metadata_json
    for key, value in episode_metadata.items():
        value_json = json.dumps(value, ensure_ascii=False, sort_keys=True)
        if key in metadata_grp:
            del metadata_grp[key]
        metadata_grp.create_dataset(key, data=np.array(value_json, dtype=dt))
...
def get_env_interface_info_from_dataset(
    ...
    for ep in demo_keys:
        ...
        assert datagen_info_key in f, "Could not find generation metadata in dataset {}. Ensure you have run eloggen prepare on this hdf5".format(dataset_path)
...
def parse_source_dataset(
    ...
    Args:
        ...
        task_spec (EG_TaskSpec instance or None): task spec object, which will be used to
...
def write_demo_to_hdf5(
    ...
    write_episode_metadata_json(ep_data_grp, episode_metadata)
    if frame_contexts is not None:
        write_frame_context_json(ep_data_grp, frame_contexts)
