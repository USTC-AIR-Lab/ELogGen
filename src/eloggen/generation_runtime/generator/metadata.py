...
class DatagenInfo(object):
    ...
    def __init__(
        ...
        self.articulated_states = None
        if articulated_states is not None:
            self.articulated_states = {
                key: np.array(value) for key, value in articulated_states.items()
            }
    ...
    def to_dict(self):
        ...
        if self.articulated_states is not None:
            ret["articulated_states"] = deepcopy(self.articulated_states)
        return ret
