...
This module provides custom pose utilities specific to ElogGen generation runtime.
...
def pose_inv(pose):
    ...
    optimized implementation while maintaining compatibility with the generation runtime's usage patterns.
...
def pose_in_A_to_pose_in_B(pose_in_A, pose_A_in_B):
    ...
    This wrapper maintains the runtime keyword-argument interface and handles
...
def quat2axisangle_separate(quat):
    ...
    NOTE: This is a wrapper around BEHAVIOR's quat2axisangle that maintains
          the runtime interface (returning axis and angle separately).
    """
...
def axisangle2quat_separate(axis, angle):
    ...
    NOTE: This is a wrapper around BEHAVIOR's axisangle2quat that maintains
          the runtime interface (accepting axis and angle separately).
    """
